...
**NEVER modify files inside `.gemini/skills/unikit-*/`.**
...
Rule: strip leading `/`. If name doesn't start with `unikit-` AND `.gemini/skills/unikit-<name>/SKILL.md` exists → use `unikit-<name>`. Otherwise use as-is.

After resolving: verify `.gemini/skills/<resolved>/SKILL.md` exists. If not → error:
"Skill '<resolved>' not found. Available unikit skills: [scan `.gemini/skills/unikit-*/SKILL.md` for installed skills]."
...
Read the base skill file: `.gemini/skills/<resolved>/SKILL.md`
...
1. Read the base `.gemini/skills/<skill>/SKILL.md`
...
1. **Skill-context only** — all rules go to `.unikit/skill-context/`, NEVER to `.gemini/skills/unikit-*/`
...
6. **Ownership boundary** — this command owns `.unikit/skill-context/*`; treats base skills (`.gemini/skills/`) and rules (`.unikit/memory/`, `.unikit/RULES.md`) as read-only
