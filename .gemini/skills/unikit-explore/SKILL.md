...
description: >-
  ...
  and clarifying requirements in Unity projects. Use when the user wants to think through
  ...
  Also use when discussing Unity architecture decisions, Zenject bindings, game system design,
...
# UniKit Explore — Thinking Partner for Unity Projects
...
3. **Read `.unikit/memory/RULES_INDEX.md`**. Load rules:
   ...
   - **Core**: read the Core table. For EACH row where Required By = `all` or contains `unikit-explore` — read that file from `.unikit/memory/core/` using the Read tool. Do NOT skip any matching row. Always re-read at skill start, never rely on prior conversation cache
   - **Stack**: load dynamically when the current task or context matches "Load When" column, or when a need arises during work
4. **`.unikit/skill-context/unikit-explore/SKILL.md`** — project-specific skill overrides (if exists)
...
Without this context you'll give generic Unity advice instead of advice grounded in this project's actual architecture, conventions, and patterns. The quality difference is enormous — this is the foundation that makes explore mode valuable.
...
4. **Generate `RESEARCH_BRIEF.md`** — a structured brief for implementation agents.
   ...
   **This step reads the template and prompt — they are NOT loaded during exploration, only at save time.**
   ...
   a. Read the template from `.gemini/skills/unikit-explore/references/explore-brief-template.md`
   b. Read the filling rules from `.gemini/skills/unikit-explore/references/explore-brief-prompt.md`
...
You: Object pooling is a broad space in Unity. Let me think about this...

     POOLING APPROACHES IN Unity
