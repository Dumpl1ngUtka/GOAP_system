...
Read `.gemini/skills/unikit-docs/references/ENGINE_RULES.md` — this file is installed by the unikit setup and contains all engine-specific detection tables, folder structure patterns, module boundary mechanism, and deep analysis patterns for the current project's engine.
...
For the README template and content guidelines → read `.gemini/skills/unikit-docs/references/DOC-TEMPLATES.md` (README section).
...
For content guidelines per document type → read `.gemini/skills/unikit-docs/references/DOC-TEMPLATES.md`.
...
For comprehensive checklists (Technical, Unity-Specific, "New User Eyes" readability, Standards Compliance) → read `.gemini/skills/unikit-docs/references/REVIEW-CHECKLISTS.md`.
...
For the complete conversion process, template placeholders, nav_links format, markdown→HTML conversion rules, and auto-features documentation → read `.gemini/skills/unikit-docs/references/HTML-GENERATION.md`.

Summary of the process:

1. Create `docs-html/` directory
2. Read `templates/html-template.html` (use as base for all pages)
3. For each markdown file → convert to HTML following conversion rules from `.gemini/skills/unikit-docs/references/HTML-GENERATION.md`
