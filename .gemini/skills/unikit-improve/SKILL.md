...
name: unikit-improve
description: >-
  Refine and enhance an existing feature plan for the current Unity project.
...
# Unity Improve - Feature Plan Refinement
...
3. **Read `.unikit/memory/RULES_INDEX.md`**. Load rules:
   ...
   - **Core**: read the Core table. For EACH row where Required By = `all` or contains `unikit-improve` — read that file from `.unikit/memory/core/` using the Read tool. Do NOT skip any matching row. Always re-read at skill start, never rely on prior conversation cache
   - **Stack**: load dynamically when the current task or context matches "Load When" column, or when a need arises during work
4. **`.unikit/skill-context/unikit-improve/SKILL.md`** — project-specific skill overrides (if exists)
