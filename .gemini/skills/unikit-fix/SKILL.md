...
name: unikit-fix
description: >-
  Fix a specific bug or problem in a Unity project. Supports two modes — immediate fix
  or plan-first. Without arguments executes existing .unikit/FIX_PLAN.md. Checks Unity
  ...
  or pastes an error message or Unity console log.
  Also trigger when user shares a NullReferenceException, MissingReferenceException,
  compilation error, or any Unity-specific error.
...
- Where it occurs (file, class, method, Unity component)
- Steps to reproduce (if provided)
- Unity console output (errors, warnings, stack traces)
...
3. Is there a stack trace or Unity Console log?
...
- Impact scope
- Related Unity systems (if applicable)
...
### 4.1 Unity Compile Check

Use UnityMCP to check that the project compiles after the fix:
