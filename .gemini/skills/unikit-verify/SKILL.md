...
description: >-
  ...
  tests pass, and Unity-specific conventions are followed (per ENGINE_RULES.md).
...
- Read `.gemini/skills/unikit-verify/references/CONTEXT-GATES-AND-OWNERSHIP.md` first.
...
Read `.gemini/skills/unikit-verify/references/ENGINE_RULES.md` to load engine-specific verification checks.
...
### 2.1 Unity Compile Check

Use UnityMCP to check that the project compiles after implementation:
- Refresh/recompile the project through UnityMCP
- Check the Unity console for compilation errors
- If errors found — display them with `file:line` references
- If UnityMCP is unavailable — skip and note: `Compilation check: UnityMCP unavailable, skipped`

### 2.2 Unity Test Check

Use UnityMCP to run tests for affected modules:
...
- If UnityMCP is unavailable — skip and note: `Test run: UnityMCP unavailable, skipped`

### 2.3 Engine-Specific Checks

Apply all checks defined in `.gemini/skills/unikit-verify/references/ENGINE_RULES.md`:
...
Apply the canonical contract from `.gemini/skills/unikit-verify/references/CONTEXT-GATES-AND-OWNERSHIP.md`.
...
### Code Quality
- Compilation: ✅ / ⏭️ UnityMCP unavailable
- Tests: ✅ 12 passed, 0 failed / ⏭️ UnityMCP unavailable
...
| Compilation (UnityMCP) | Reported if available | **Required** to pass if UnityMCP is available |
| Tests (UnityMCP) | Reported if available | **Required** to pass if test assemblies exist for affected modules |
...
- Context gates (Architecture, Rules — see thresholds in `.gemini/skills/unikit-verify/references/CONTEXT-GATES-AND-OWNERSHIP.md`)
