...
3. **Read `.unikit/skill-context/unikit-memory/SKILL.md`** — MANDATORY if the file exists.
...
2. Run 1-3 targeted `WebSearch` queries to gather material:
   - `"<topic> best practices Unity"` — recommended approaches
   - `"<topic> common mistakes"` — pitfalls to document
   - `"<topic> cheat sheet"` or `"<topic> API reference"` — concise reference material
...
Keep only information relevant to using the framework in a Unity/C# project. Discard web-framework specifics, non-Unity platforms, and irrelevant content. Transform passive documentation into actionable rules ("Use X when..." instead of "X is a feature that...").
...
3. **Detect conflicts** (skip for New File candidates) — read the target rule file and check whether the RULES.md entry **contradicts** an existing rule. Mark each candidate as:
   ...
   - **Override** — contradicts an existing rule in the target file (e.g., "Never use var" overrides a C# convention, "Factory classes instead of Zenject PlaceholderFactory" overrides a DI pattern). Record which specific rule(s) in the target file conflict.
