...
name: unikit-plan
description: >-
  Create a detailed feature plan for a Unity project.
...
# Unity Feature Plan Generator

Create a structured feature plan and roadmap for the current Unity project.
...
3. **Read `.unikit/memory/RULES_INDEX.md`**. Load rules:
   ...
   - **Core**: read the Core table. For EACH row where Required By = `all` or contains `unikit-plan` — read that file from `.unikit/memory/core/` using the Read tool. Do NOT skip any matching row. Always re-read at skill start, never rely on prior conversation cache
   - **Stack**: load dynamically when the current task or context matches "Load When" column, or when a need arises during work
4. **`.unikit/skill-context/unikit-plan/SKILL.md`** — project-specific skill overrides (if exists)
...
Return format: structured report matching the Plan Brief Template sections from `.gemini/skills/unikit-plan/references/TASK-FORMAT.md`. Do not guess — base on actual code read. Thoroughness: very thorough.
...
Use the canonical templates from `.gemini/skills/unikit-plan/references/TASK-FORMAT.md`.
...
**Always created** — the plan's own technical brief based on the current codebase state at planning time. Use Plan Brief Template from `.gemini/skills/unikit-plan/references/TASK-FORMAT.md`. Content comes from Step 4 Phase B, synthesized with Bootstrap rules. Do not invent — base on actual codebase patterns.
...
2. INTERFACES — full C# signatures for every interface in tasks
