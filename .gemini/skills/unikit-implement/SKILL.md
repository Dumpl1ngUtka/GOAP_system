...
After all tasks in a phase are done, check Unity console for compilation errors.

**Prerequisite:** Check if UnityMCP is configured in `.gemini/settings.json` at the project root. If UnityMCP is not present in MCP settings — skip this step entirely and proceed to 3.7.

**If UnityMCP is available:**

1. Read the Unity console log via UnityMCP, filtering for errors
